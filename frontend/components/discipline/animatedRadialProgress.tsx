...
const AnimatedRadialProgress = ({ value }: { value: number }) => {
  ...
  return (
    <svg className="progress-ring" width="120" height="120">
      <circle className="progress-ring__circle" stroke="#3B82F6" strokeWidth="8" fill="transparent" r="45" cx="60" cy="60" style={{
        strokeDasharray: `${2 * Math.PI * 45} ${2 * Math.PI * 45}`,
        strokeDashoffset: strokeDashoffset,
        transition: `stroke-dashoffset 0.3s ease-in-out`,
        transitionDelay: `${prevStrokeDashoffset === strokeDashoffset ? '0s' : '0.1s'}`,
      }} />
      <text x="50%" y="50%" textAnchor="middle" fill="#3B82F6" dy=".3em" fontSize="20px">{value}%</text>
    </svg>
  );
};
